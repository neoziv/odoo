...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('neoziv.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', r'^Sampling from a set', category=DeprecationWarning, module='neoziv')
...
DEFAULT_LOG_CONFIGURATION = [
    'neoziv.http.rpc.request:INFO',
    'neoziv.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['neoziv:DEBUG', 'neoziv.sql_db:INFO', 'neoziv.http.rpc:DEBUG'],
    'debug_rpc': ['neoziv:DEBUG', 'neoziv.sql_db:INFO', 'neoziv.http.rpc.request:DEBUG'],
    'debug': ['neoziv:DEBUG', 'neoziv.sql_db:INFO'],
    'debug_sql': ['neoziv.sql_db:DEBUG'],
    'info': [],
    'runbot': ['neoziv:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['neoziv:WARNING', 'werkzeug:WARNING'],
    'error': ['neoziv:ERROR', 'werkzeug:ERROR'],
    'critical': ['neoziv:CRITICAL', 'werkzeug:CRITICAL'],
}
