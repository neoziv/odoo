#neoziv.loggers.handlers. -*- coding: utf-8 -*-
...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.neozivrc (or to ~/.openerp_serverrc if it exists)")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store neoziv data")
        ...
        temp_tests_dir = os.path.join(tempfile.gettempdir(), 'neoziv_tests')
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "neoziv.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-request', action="append_const", dest="log_handler", const="neoziv.http.rpc.request:DEBUG", help='shortcut for --log-handler=neoziv.http.rpc.request:DEBUG')
        group.add_option('--log-response', action="append_const", dest="log_handler", const="neoziv.http.rpc.response:DEBUG", help='shortcut for --log-handler=neoziv.http.rpc.response:DEBUG')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="neoziv.http:DEBUG", help='shortcut for --log-handler=neoziv.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="neoziv.sql_db:DEBUG", help='shortcut for --log-handler=neoziv.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options. ",
            "Use these options to translate neoziv to another language. "
    ...
    def parse_config(self, args=None):
        ...
        This method initializes neoziv.tools.config and openerp.conf (the
        ...
        Typical usage of this method:
            ...
            neoziv.tools.config.parse_config(sys.argv[1:])
        """
        opt = self._parse_config(args)
        neoziv.netsvc.init_logger()
        self._warn_deprecated_options()
        neoziv.modules.module.initialize_sys_path()
        return opt
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'neoziv.conf')
        else:
            rcfilepath = os.path.expanduser('~/.neozivrc')
            old_rcfilepath = os.path.expanduser('~/.openerp_serverrc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.neozivrc' and '.openerp_serverrc' in your path. Please keep only one of "\
                "them, preferably '.neozivrc'.")

            if not os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath):
                rcfilepath = old_rcfilepath

        self.rcfile = os.path.abspath(
            self.config_file or opt.config or os.environ.get('neoziv_RC') or os.environ.get('OPENERP_SERVER') or rcfilepath)
        self.load()
    ...
    def _is_addons_path(self, path):
        from neoziv.modules.module import MANIFEST_NAMES
