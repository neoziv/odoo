...
import neoziv
import neoziv.addons
# get_encodings, ustr and exception_to_unicode were originally from tools.misc.
# There are moved to loglevels until we refactor tools.
from neoziv.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if neoziv.tools.config['db_host']:
        env['PGHOST'] = neoziv.tools.config['db_host']
    if neoziv.tools.config['db_port']:
        env['PGPORT'] = str(neoziv.tools.config['db_port'])
    if neoziv.tools.config['db_user']:
        env['PGUSER'] = neoziv.tools.config['db_user']
    if neoziv.tools.config['db_password']:
        env['PGPASSWORD'] = neoziv.tools.config['db_password']
    return env
...
def _fileopen(path, mode, basedir, pathinfo, basename=None):
    ...
    paths = neoziv.addons.__path__ + [config['root_path']]
...
def scan_languages():
    ...
    :rtype: [(str, unicode)]
    """
    csvpath = neoziv.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('neoziv.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('neoziv.foo.bar'):
            do_suff()
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = neoziv.fields.Datetime.from_string(value)
            value = neoziv.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = neoziv.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = neoziv.fields.Datetime.from_string(value)
    else:
        timestamp = value
