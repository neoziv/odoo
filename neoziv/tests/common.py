# -*- coding: utf-8 -*-
"""
The module :mod:`neoziv.tests.common` provides unittest test cases and a few
...
import neoziv
from neoziv import api
from neoziv.models import BaseModel
from neoziv.osv.expression import normalize_domain, TRUE_LEAF, FALSE_LEAF
from neoziv.service import security
from neoziv.sql_db import Cursor
from neoziv.tools import float_compare, single_email_re
from neoziv.tools.misc import find_in_path
...
# The neoziv library is supposed already configured.
ADDONS_PATH = neoziv.tools.config['addons_path']
HOST = '127.0.0.1'
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = neoziv.SUPERUSER_ID


def get_db_name():
    db = neoziv.tools.config['db_name']
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding neoziv module name.
    """
    def register(func):
        # register func by neoziv module name
        if func.__module__.startswith('neoziv.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most neoziv operations;
    """
    if not login:
...
# Main classes
# ------------------------------------------------------------
class neozivSuite(unittest.suite.TestSuite):
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('neoziv.addons.'):
...
class BaseCase(TreeCase, MetaCase('DummyCase', (object,), {})):
    ...
    longMessage = True      # more verbose error message by default: https://www.neoziv.com/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~neoziv.models.BaseModel`
        """
    ...
    def assertQueries(self, expected, flush=True):
        ...
        with patch('neoziv.sql_db.Cursor.execute', execute):
            with patch('neoziv.osv.expression.get_unaccent_wrapper', get_unaccent_wrapper):
        ...
        if self.warm:
            ...
            with self.subTest(), patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/neoziv/addons/" in filename:
                        filename = filename.rsplit("/neoziv/addons/", 1)[1]
                    if count > expected:
...
class TransactionCase(BaseCase):
    ...
    def setUp(self):
        ...
        self.registry = neoziv.registry(get_db_name())
        ...
        #: :class:`~neoziv.api.Environment` for the current test case
        self.env = api.Environment(self.cr, neoziv.SUPERUSER_ID, {})
...
class SingleTransactionCase(BaseCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.registry = neoziv.registry(get_db_name())
        ...
        cls.env = api.Environment(cls.cr, neoziv.SUPERUSER_ID, {})
...
class ChromeBrowser():
    ...
    def __init__(self, logger, window_size, test_class):
        ...
        self.request_id = 0
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_neoziv')
        self.chrome_pid = None

        otc = neoziv.tools.config
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way neoziv widgets are created they all appear as Class(...)
...
class HttpCaseCommon(BaseCase):
    ...
    def __init__(self, methodName='runTest'):
        ...
        self.xmlrpc_url = url_8 = 'http://%s:%d/xmlrpc/2/' % (HOST, neoziv.tools.config['http_port'])
    ...
    def url_open(self, url, data=None, files=None, timeout=10, headers=None, allow_redirects=True):
        self.env['base'].flush()
        if url.startswith('/'):
            url = "http://%s:%s%s" % (HOST, neoziv.tools.config['http_port'], url)
        if data or files:
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('neoziv.service.http.request.')]
        ...
        if request_threads:
            ...
            neoziv.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=True)
        neoziv.http.root.session_store.save(self.session)

    def authenticate(self, user, password):
        db = get_db_name()
        if getattr(self, 'session', None):
            neoziv.http.root.session_store.delete(self.session)

        self.session = session  = neoziv.http.root.session_store.new()
        session.db = db
        ...
        neoziv.http.root.session_store.save(session)
    ...
    def browser_js(self, url_path, code, ready='', login=None, timeout=60, **kw):
        ...
        try:
            ...
            base_url = "http://%s:%s" % (HOST, neoziv.tools.config['http_port'])
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "neoziv.startTour('%s'%s)" % (tour_name, step_delay))
        ready = kwargs.pop('ready', "neoziv.__DEBUG__.services['web_tour.tour'].tours['%s'].ready" % tour_name)
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    :class:`~neoziv.fields.Many2one` fields assign a singleton recordset::
    ...
    For :class:`~neoziv.fields.Many2many` fields, the field itself is a
    :class:`~neoziv.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~neoziv.fields.One2many` are reified as
    :class:`~neoziv.tests.common.O2MProxy`.

    Because the :class:`~neoziv.fields.One2many` only exists through its
    parent, it is manipulated more directly by creating "sub-forms"
    with the :meth:`~neoziv.tests.common.O2MProxy.new` and
    :meth:`~neoziv.tests.common.O2MProxy.edit` methods. These would
    ...
    :type recordp: neoziv.models.Model
    ...
    :type view: int | str | neoziv.model.Model
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~neoziv.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~neoziv.fields.One2many` record.
...
def record_to_values(fields, record):
    ...
    for f, v in record.read(to_read)[0].items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = neoziv.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = neoziv.fields.Date.to_string(v)
        r[f] = v
    return r
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return neoziv.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return neoziv.fields.Date.to_string(value)
    return value
...
def tagged(*tags):
    ...
    By default, all Test classes from neoziv.tests.common have a test_tags
