...
import neoziv
from neoziv.tools import topological_sort, unique
from neoziv.netsvc import init_logger
from neoziv.tests import standalone_tests
import neoziv.tests.loader

_logger = logging.getLogger('neoziv.tests.test_module_operations')
...
def install(db_name, module_id, module_name):
    with neoziv.api.Environment.manage():
        with neoziv.registry(db_name).cursor() as cr:
            env = neoziv.api.Environment(cr, neoziv.SUPERUSER_ID, {})
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall cycle of neoziv modules")
    ...
    parser.add_argument("--addons-path", "-p", type=str,
        help="Comma-separated list of paths to directories containing extra neoziv modules")
...
def test_full(args):
    ...
    with neoziv.api.Environment.manage():
        with neoziv.registry(args.database).cursor() as cr:
            env = neoziv.api.Environment(cr, neoziv.SUPERUSER_ID, {})
...
def test_scripts(args):
    ...
    registry = neoziv.registry(args.database)
    for module_name in registry._init_modules:
        # import tests for loaded modules
        neoziv.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        neoziv.tools.config['addons_path'] = ','.join([args.addons_path, neoziv.tools.config['addons_path']])
        if args.data_dir:
            neoziv.tools.config['data_dir'] = args.data_dir
        neoziv.modules.module.initialize_sys_path()

    init_logger()
    logging.getLogger('neoziv.modules.loading').setLevel(logging.CRITICAL)
    logging.getLogger('neoziv.sql_db').setLevel(logging.CRITICAL)
