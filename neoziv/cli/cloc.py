...
from neoziv.tools import cloc, config
from . import Command

class Cloc(Command):
    """\
    neoziv cloc is a tool to count the number of relevant lines written in
    ...
    It has two modes of operation, either by providing a path:
        ...
        neoziv-bin cloc -p module_path

    Or by providing the name of a database:

        neoziv-bin cloc --addons-path=dirs -d database
