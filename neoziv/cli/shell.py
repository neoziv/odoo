...
class Shell(Command):
    """Start neoziv in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        neoziv.cli.server.report_configuration()
        neoziv.service.server.start(preload=[], stop=True)
        ...
        with neoziv.api.Environment.manage():
            if dbname:
                ...
                with registry.cursor() as cr:
                    ...
                    ctx = neoziv.api.Environment(cr, uid, {})['res.users'].context_get()
                    env = neoziv.api.Environment(cr, uid, ctx)
                    local_vars['env'] = env
