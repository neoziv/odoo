...
__author__ = neoziv.release.author
__version__ = neoziv.release.version

# Also use the `neoziv` logger for the main script.
_logger = logging.getLogger('neoziv')
...
def report_configuration():
    ...
    """
    config = neoziv.tools.config
    _logger.info("neoziv version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', neoziv.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = neoziv.tools.config
    if not neoziv.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = neoziv.modules.registry.Registry.new(dbname)
        with neoziv.api.Environment.manage():
            with registry.cursor() as cr:
                neoziv.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with neoziv.api.Environment.manage():
        with registry.cursor() as cr:
            neoziv.tools.trans_load(
                cr, config["translate_in"], config["language"], overwrite=overwrite,
            )

def main(args):
    check_root_user()
    neoziv.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                neoziv.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except neoziv.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        neoziv.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = neoziv.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the neoziv server (default command)"""
    def run(self, args):
        main(args)
