...
RPC_VERSION_1 = {
        'server_version': neoziv.release.version,
        'server_version_info': neoziv.release.version_info,
        'server_serie': neoziv.release.serie,
        'protocol_version': 1,
}
...
def exp_authenticate(db, login, password, user_agent_env):
    if not user_agent_env:
        user_agent_env = {}
    res_users = neoziv.registry(db)['res.users']
    try:
...
def exp_about(extended=False):
    ...
    if extended:
        return info, neoziv.release.version
    return info

def exp_set_loglevel(loglevel, logger=None):
    # TODO Previously, the level was set on the now deprecated
    # `neoziv.netsvc.Logger` class.
    return True
