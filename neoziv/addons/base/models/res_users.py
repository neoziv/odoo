...
from neoziv.exceptions import AccessDenied, AccessError, UserError, ValidationError
from neoziv.http import request
from neoziv.modules.module import get_module_resource
from neoziv.osv import expression
from neoziv.service.db import check_super
from neoziv.tools import partition, collections, frozendict, lazy_property, image_process
...
class Users(models.Model):
    ...
    def _get_default_image(self):
        """ Get a default image when the user is created without image
            ...
            https://github.com/neoziv/neoziv/blob/11.0/neoziv/addons/base/res/res_partner.py
        """
    ...
    def unlink(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by neoziv (updates, module installation, ...)'))
        self.clear_caches()
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: neoziv.exceptions.AccessDenied when old password is wrong
        :raise: neoziv.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
        if not new_passwd:
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If your neoziv is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "neoziv in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.neoziv.com/documentation/14.0/administration/deployment/deploy.html#https for details.",
                    source
                )
