...
from neoziv.tests.common import BaseCase, SavepointCase, TransactionCase
from neoziv.tools import (
...
class EmailConfigCase(SavepointCase):
    @patch.dict("neoziv.tools.config.options", {"email_from": "settings@example.com"})
    ...
    def test_email_from_rewrite(self):
        ...
        set_param('mail.catchall.domain', 'neoziv.example.com')
        ...
        set_param('mail.force.smtp.from', False)
        set_param('mail.dynamic.smtp.from', 'notification@neoziv.example.com')
        set_param('mail.catchall.domain', 'neoziv.example.com')

        email_from, return_path = get_email_from('"Admin" <admin@test.example.com>')
        self.assertEqual(email_from, '"Admin (admin@test.example.com)" <notification@neoziv.example.com>',
                         msg='Domain is not the same as the catchall domain, we should force the email FROM')
        self.assertEqual(return_path, 'notification@neoziv.example.com')

        email_from, return_path = get_email_from('"Admin" <admin@neoziv.example.com>')
        self.assertEqual(email_from, '"Admin" <admin@neoziv.example.com>',
        ...
        set_param('mail.force.smtp.from', False)
        set_param('mail.dynamic.smtp.from', '"Your notification bot" <notification@neoziv.example.com>')
        set_param('mail.catchall.domain', 'neoziv.example.com')
