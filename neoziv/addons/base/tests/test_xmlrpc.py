...
from neoziv.exceptions import AccessDenied, AccessError
from neoziv.http import _request_stack

import neoziv.tools
from neoziv.tests import common
from neoziv.service import common as auth, model
...
class TestXMLRPC(common.HttpCase):
    ...
    def _json_call(self, *args):
        self.opener.post("http://%s:%s/jsonrpc" % (common.HOST, neoziv.tools.config['http_port']), json={
            'jsonrpc': '2.0',
            'id': None,
            'method': 'call',
            'params': {
