...
class MetaModel(api.Meta):
    ...
    def __init__(self, name, bases, attrs):
        ...
        if not hasattr(self, '_module'):
            assert self.__module__.startswith('neoziv.addons.'), \
                "Invalid import of %s.%s, it should start with 'neoziv.addons'." % (self.__module__, name)
...
class BaseModel(MetaModel('DummyModel', (object,), {'_register': False})):
    """Base class for neoziv models.

    neoziv models are created by inheriting one of the following:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    the :attr:`~neoziv.models.BaseModel._register` attribute may be set to False.
    """
    __slots__ = ['env', '_ids', '_prefetch_ids']

    _auto = False
    """Whether a database table should be created.
    If set to ``False``, override :meth:`~neoziv.models.BaseModel.init`
    ...
    .. tip:: To create a model without any table, inherit
            from :class:`~neoziv.models.AbstractModel`.
    """
    ...
    .. warning::
      ...
      :attr:`~neoziv.models.Model._inherits`-ed models, the inherited field will
    ...
    def compute_concurrency_field(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = neoziv.fields.Datetime.now()

    @api.depends('create_date', 'write_date')
    def compute_concurrency_field_with_access(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = \
                record.write_date or record.create_date or neoziv.fields.Datetime.now()
    ...
    def load(self, fields, data):
        ...
        for field_path in fields:
            ...
            if isinstance(model_fields[field_path[0]], neoziv.fields.Many2one):
            ...
            for field_name in field_path:
                ...
                if isinstance(model_fields[field_name], neoziv.fields.One2many):
    ...
    def _add_fake_fields(self, fields):
        from neoziv.fields import Char, Integer
...
%(fields_list)s""") % {
    ...
    def _check_company(self, fnames=None):
        ...
        :raises UserError: if the `company_id` of the value of any field is not
            ...
            :class:`~neoziv.addons.base.models.res_company`).

        For :class:`~neoziv.addons.base.models.res_users` relational fields,
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~neoziv.fields.Integer`,
          :class:`~neoziv.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~neoziv.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~neoziv.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~neoziv.fields.Many2one`, the value should be the
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~neoziv.fields.Date` and
              :class:`~neoziv.fields.Datetime` fields use strings as values
              (written and read) rather than :class:`~python:datetime.date` or
              :class:`~python:datetime.datetime`. These date strings are
              UTC-only and formatted according to
              :const:`neoziv.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
              :const:`neoziv.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
        * .. _openerp/models/relationals/format:

          :class:`~neoziv.fields.One2many` and
          :class:`~neoziv.fields.Many2many` use a special "commands" format to
    ...
    def ensure_one(self):
        ...
        :raise neoziv.exceptions.ValueError: ``len(self) != 1``
        """
        try:
    ...
    def with_env(self, env):
        ...
        :param env:
        :type env: :class:`~neoziv.api.Environment`
    ...
    def with_company(self, company):
        ...
        :type company: :class:`~neoziv.addons.base.models.res_company` or int
        ...
        for field, value in sorted(field_values, key=is_monetary):
            ...
            if field.relational:
                ...
                for invf in self._field_inverses[field]:
                    ...
                    # /home/dle/src/neoziv/master-nochange-fp/neoziv/addons/test_new_api/tests/test_new_fields.py
    ...
    def _populate_factories(self):
        ...
        See :mod:`neoziv.tools.populate` for population tools and applications.
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted neoziv models.

    neoziv models are created by inheriting from this class::
