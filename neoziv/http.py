...
# *broken*
NO_POSTMORTEM = (neoziv.exceptions.AccessDenied,
                 neoziv.exceptions.UserError,
                 neoziv.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                neoziv.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = neoziv.service.common.dispatch
        elif service_name == 'db':
            dispatch = neoziv.service.db.dispatch
        elif service_name == 'object':
            dispatch = neoziv.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                neoziv.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                neoziv.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except Exception as e:
        _logger.exception(neoziv.tools.exception_to_unicode(e))
        neoziv.tools.debugger.post_mortem(neoziv.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    """ Parent class for all neoziv Web request types, mostly deals with
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in neoziv.service.wsgi_server.application
        if self.db:
    ...
    @property
    def cr(self):
        """ :class:`~neoziv.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~neoziv.api.Environment` bound to current request. """
        if self._env is None:
            self._env = neoziv.api.Environment(self.cr, self.uid, self.context)
        return self._env
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            neoziv.tools.debugger.post_mortem(
                neoziv.tools.config, sys.exc_info())
    ...
    def registry(self):
        ...
        .. deprecated:: 8.0
            ...
            use :attr:`.env`
        """
        return neoziv.registry(self.db)
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        neoziv implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <neoziv.http.WebRequest.csrf_token`, the
          :data:`~neoziv.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          neoziv) as e.g. it is a REST API or a `webhook
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, SessionExpiredException):
                ...
                elif isinstance(exception, (neoziv.exceptions.UserError,
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "neoziv Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "neoziv Session Expired"
...
neoziv URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.neoziv.com/documentation/14.0/developer/reference/http.html#csrf for
more details.

* if this endpoint is accessed through neoziv via py-QWeb form, embed a CSRF
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["neoziv", "addons"]:
            module = ""
        else:
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('neoziv.addons.') and c.__module__.split(".")[2] in modules
...
class OpenERPSession(sessions.Session):
    ...
    def authenticate(self, db, login=None, password=None):
        ...
        uid = neoziv.registry(db)['res.users'].authenticate(db, login, password, env)
        self.pre_uid = uid
    ...
    def check_security(self):
        ...
        env = neoziv.api.Environment(request.cr, self.uid, self.context)
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or neoziv.SUPERUSER_ID)
...
class Root(object):
    ...
    def session_store(self):
        # Setup http sessions
        path = neoziv.tools.config.session_dir
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint, routing in neoziv.http._generate_routing_rules([''] + neoziv.conf.server_wide_modules, True):
    ...
    def load_addons(self):
        ...
        statics = {}
        for addons_path in neoziv.addons.__path__:
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    try:
                        neoziv.registry(db).check_signaling()
                        with neoziv.tools.mute_logger('neoziv.sql_db'):
...
def db_list(force=False, httprequest=None):
    dbs = neoziv.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if neoziv.tools.config['dbfilter']:
        d, h = re.escape(d), re.escape(h)
        r = neoziv.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
        dbs = [i for i in dbs if re.match(r, i)]
    elif neoziv.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, neoziv will
        # use the value of --database as a comma seperated list of exposed databases.
        exposed_dbs = set(db.strip() for db in neoziv.tools.config['db_name'].split(','))
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = neoziv.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = neoziv.tools.ustr(filename)
