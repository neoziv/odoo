...
from neoziv.exceptions import CacheMiss
...
class Field(MetaField('DummyField', (object,), {})):
    ...
    :param str group_operator: aggregate function used by :meth:`~neoziv.models.Model.read_group`
    ...
    def mapped(self, records):
        ...
        over a simple call to `~neoziv.models.BaseModel.mapped()` on a recordset.
        """
        if self.name == 'id':
            # not stored in cache
...
class Float(Field):
    ...
    :param digits: a pair (total, decimal) or a string referencing a
        :class:`~neoziv.addons.base.models.decimal_precision.DecimalPrecision` record name.
    ...
    :func:`~neoziv.fields.Float.round()` to round a float with the given precision.
    :func:`~neoziv.fields.Float.is_zero()` to check if a float equals zero at the given precision.
    :func:`~neoziv.fields.Float.compare()` to compare two floats at the given precision.
...
class Monetary(Field):
    ...
    :class:`res_currency<neoziv.addons.base.models.res_currency.Currency>`.
    ...
    :param str currency_field: name of the :class:`Many2one` field
        holding the :class:`res_currency <neoziv.addons.base.models.res_currency.Currency>`
    ...
    def convert_to_cache(self, value, record, validate=True):
        ...
        if value and validate:
            # FIXME @rco-neoziv: currency may not be already initialized if it is
...
class Image(Binary):
    ...
    :param bool verify_resolution: whether the image resolution should be verified
        ...
        See :class:`neoziv.tools.image.ImageProcess` for maximum image resolution (default: ``45e6``).
...
class Many2one(_Relational):
    ...
    :param bool check_company: Mark the field to be verified in
        :meth:`~neoziv.models.Model._check_company`. Add a default company
